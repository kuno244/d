...
s=default_save(now=1000); assert validate_save(s)==[]; assert s['save_version']==4
# Historical v0 -> v1 -> v2 -> v3 -> v4 path remains supported.
...
m=migrate_save(old,now=2000); assert m['save_version']==4 and 'world_state' in m and 'capacities' in m
...
m1=migrate_save(v1,now=2000); assert m1['save_version']==4 and m1['resources']['food']==321 and m1['city_state']['buildings']
with tempfile.TemporaryDirectory() as td:
    ...
    recovered=recover_json(p); assert recovered['save_version']==4
